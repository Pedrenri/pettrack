        ...
        <div className="w-full max-w-3xl space-y-8">
            ...
            <div className="grid grid-cols-3 gap-4">
              {animal.animal_photos.map((p: any) => (
                ...
                >
                  <img
                    src={p.url}
                    className="aspect-square rounded-xl object-cover border hover:opacity-90 transition"
                  />
                </button>
              ))}
            </div>
          ...
          <section className="grid sm:grid-cols-2 gap-4 bg-white rounded-2xl p-6 shadow">
            ...
            <Item
              ...
              value={formatDate(animal.last_fed)}
            />
            <Item
              label="Último manejo"
              value={formatDate(animal.last_handled)}
            />
            <Item
              label="Última troca de pele"
              value={formatDate(animal.last_shed)}
            />
            <Item
              label="Última pesagem"
              value={formatDate(animal.last_weighed)}
            />
          </section>
          ...
          <section className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-700">
                Este QR Code leva para esta página pública. Pode ser impresso ou
                usado em identificação física.
              </p>
            </div>
          </section>
        </div>
